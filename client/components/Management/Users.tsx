...
const Users: React.FC<UsersProps> = ({ theme = 'dark' }) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get(`https://superadminapi.ristestate.com/api/users?search=${searchTerm}`, {
  ...
  const handleViewUser = async (user: Customer) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await axios.get(`https://superadminapi.ristestate.com/api/users/${user.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
