...
const Branches: React.FC<BranchesProps> = ({ theme = 'dark' }) => {
  ...
  const fetchBranches = async () => {
    try {
      ...
      const response = await axios.get('https://superadminapi.ristestate.com/api/branches', {
  ...
  const fetchBranchProducts = async (branchId: string) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await axios.get(`https://superadminapi.ristestate.com/api/branches/${branchId}/products`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
  ...
  const toggleBusyMode = async (id: string, currentBusy: boolean) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await axios.patch(`https://superadminapi.ristestate.com/api/branches/${id}/busy`,
        { is_busy: !currentBusy },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.success) {
  ...
  const toggleStatus = async (id: string, currentStatus: string) => {
    try {
      ...
      const token = localStorage.getItem('authToken');
      const response = await axios.patch(`https://superadminapi.ristestate.com/api/branches/${id}/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.success) {
