...
const FISCAL_STREAM_DATA = [
  { month: 'Jan', revenue: 84000, expected: 75000, commission: 4200 },
  { month: 'Feb', revenue: 95000, expected: 85000, commission: 4750 },
  { month: 'Mar', revenue: 142000, expected: 130000, commission: 7100 },
  { month: 'Apr', revenue: 129000, expected: 145000, commission: 6450 },
  { month: 'May', revenue: 164000, expected: 160000, commission: 8200 },
  { month: 'Jun', revenue: 212000, expected: 195000, commission: 10600 },
  { month: 'Jul', revenue: 184000, expected: 210000, commission: 9200 },
];

const INSTITUTION_PERFORMANCE = [
  { name: 'Freshway Mart', revenue: 384000, branches: 12, growth: '+14.2%', impact: 88, color: '#10b981' },
  { name: 'Shoa Shopping', revenue: 292200, branches: 8, growth: '+11.8%', impact: 72, color: '#3b82f6' },
  { name: 'Vivat Market', revenue: 164100, branches: 5, growth: '-2.4%', impact: 45, color: '#f43f5e' },
  { name: 'Bambi Mart', revenue: 92000, branches: 2, growth: '+22.1%', impact: 32, color: '#a855f7' },
  { name: 'Safeway Central', revenue: 78000, branches: 3, growth: '+4.5%', impact: 28, color: '#06b6d4' },
];

const TOP_PRODUCTS_DATA = [
  { name: 'Organic Dairy', value: 35, color: '#10b981' },
  { name: 'Premium Meat', value: 25, color: '#3b82f6' },
  { name: 'Fresh Produce', value: 20, color: '#a855f7' },
  { name: 'Artisanal Bakery', value: 12, color: '#f59e0b' },
        ...
        <div className={`lg:col-span-2 glass-card p-10 shadow-2xl relative overflow-hidden transition-all ${isDark ? 'border-white/5' : 'border-black/10 bg-white'}`}>
          ...
          <div className="h-80 w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={FISCAL_STREAM_DATA}>
                <defs>
        ...
        <div className={`glass-card p-10 shadow-2xl flex flex-col transition-all ${isDark ? 'bg-white/[0.02] border-white/5' : 'bg-white border-black/10'}`}>
          ...
          <div className="h-64 w-full relative mb-10">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className={`text-3xl font-black transition-colors ${isDark ? 'text-white' : 'text-gray-950'}`}>82%</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className={`glass-card p-10 shadow-2xl transition-all flex flex-col ${isDark ? 'bg-white/[0.02] border-white/5' : 'bg-white border-black/10'}`}>
           ...
           <div className="flex-1 space-y-4">
              {INSTITUTION_PERFORMANCE.sort((a,b) => b.revenue - a.revenue).slice(0, 4).map((inst, idx) => (
                <div key={idx} className={`flex items-center justify-between p-5 rounded-[2rem] border transition-all hover:translate-x-2 group ${isDark ? 'bg-white/[0.02] border-white/5 hover:bg-white/5' : 'bg-gray-50 border-black/5 hover:bg-gray-100 shadow-sm'}`}>
                   ...
                   <div className="text-right">
                      <p className={`text-base font-black transition-colors ${isDark ? 'text-white' : 'text-gray-950'}`}>ETB {(inst.revenue/1000).toFixed(0)}K</p>
        ...
        <div className={`glass-card p-10 shadow-2xl transition-all relative overflow-hidden ${isDark ? 'bg-white/[0.02] border-white/5' : 'bg-white border-black/10'}`}>
          ...
          <div className="space-y-6 relative z-10">
             {INSTITUTION_PERFORMANCE.map((item, idx) => (
