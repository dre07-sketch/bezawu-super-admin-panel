...
interface Supermarket {
  ...
  branches: number;
  status: 'OPERATIONAL' | 'PENDING' | 'SUSPENDED';
...
const MOCK_DATA: Supermarket[] = [
  {
    id: '1',
    name: 'Freshway Mart',
    logo: 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=128&h=128&fit=crop',
    tin: '0098734211',
    email: 'ops@freshway.et',
    phone: '+251 911 223 344',
    website: 'www.freshway.et',
    reg_code: 'BEZ-8821-X',
    vat_cert: 'https://images.unsplash.com/photo-1568667256549-094345857637?w=800&q=80',
    business_license: 'https://images.unsplash.com/photo-1554224155-1696413565d3?w=800&q=80',
    branches: 12,
    status: 'OPERATIONAL',
    total_revenue: 1240500.75,
    total_customers: 452,
    total_inventory: 4,
    bankAccounts: [
      { id: 'b1', bank_name: 'Commercial Bank of Ethiopia', account_name: 'Freshway Retail PLC', account_number: '100023445566', is_primary: true }
    ]
  },
  {
    id: '2',
    name: 'Shoa Shopping',
    logo: 'https://images.unsplash.com/photo-1534723452862-4c874018d66d?w=128&h=128&fit=crop',
    tin: '0021345511',
    email: 'contact@shoa.et',
    phone: '+251 912 334 556',
    website: 'www.shoa.et',
    reg_code: 'BEZ-1029-Y',
    vat_cert: 'https://images.unsplash.com/photo-1568667256549-094345857637?w=800&q=80',
    business_license: 'https://images.unsplash.com/photo-1554224155-1696413565d3?w=800&q=80',
    branches: 5,
    status: 'OPERATIONAL',
    total_revenue: 890200.00,
    total_customers: 318,
    total_inventory: 12,
    bankAccounts: [
      { id: 'b2', bank_name: 'Awash Bank', account_name: 'Shoa Supermarket SC', account_number: '013204556677', is_primary: true }
    ]
  },
  {
    id: '3',
    name: 'Bambi Mart',
    logo: 'https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=128&h=128&fit=crop',
    tin: '0011223344',
    email: 'hello@bambi.mart',
    phone: '+251 944 112 233',
    website: 'www.bambimart.com',
    reg_code: 'BEZ-4562-Z',
    vat_cert: 'https://images.unsplash.com/photo-1568667256549-094345857637?w=800&q=80',
    business_license: 'https://images.unsplash.com/photo-1554224155-1696413565d3?w=800&q=80',
    branches: 1,
    ...
    total_inventory: 1,
    bankAccounts: [
      { id: 'b3', bank_name: 'Dashen Bank', account_name: 'Bambi Mart Enterprise', account_number: '9928345110', is_primary: true }
    ]
  }
];
      ...
      <div className={`relative w-full max-w-7xl glass-card overflow-hidden shadow-[0_0_150px_rgba(0,0,0,0.5)] border transition-all duration-500 flex flex-col max-h-[96vh] ${isDark ? 'bg-[#020617]/40 border-white/10' : 'bg-white/40 border-black/5'}`}>
          ...
          <div className="flex items-center gap-6 z-10">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <div className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest ${selectedEntity.status === 'OPERATIONAL' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                   ...
                   {[ { label: 'VAT Cert', img: selectedEntity.vat_cert }, { label: 'License', img: selectedEntity.business_license } ].map((doc, i) => (
                     <div key={i} className="space-y-3 group">
                       <div className={`relative aspect-[3/4] rounded-[2rem] overflow-hidden border transition-all duration-700 group-hover:scale-[1.02] shadow-xl ${isDark ? 'border-white/10 bg-white/5' : 'border-black/5 bg-white/40'}`}>
                         <img src={doc.img} alt={doc.label} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                         <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <button className="w-12 h-12 rounded-full bg-emerald-500 text-white flex items-center justify-center shadow-2xl transform translate-y-4 group-hover:translate-y-0 transition-all">
                               <Eye size={20} />
                            </button>
                         </div>
                       </div>
        ...
        <div className={`p-8 border-t flex items-center justify-between gap-8 ${isDark ? 'border-white/10 bg-black/40' : 'border-black/5 bg-gray-50'}`}>
          ...
          <div className="flex gap-3">
             <button onClick={closeModal} className={`px-8 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${isDark ? 'border-rose-500/20 text-rose-400 hover:bg-rose-500/10' : 'border-rose-200 text-rose-600 hover:bg-rose-50'}`}>Decline</button>
             <button className="px-10 py-3 btn-primary text-[9px] font-black uppercase tracking-widest rounded-xl shadow-lg">Authorize</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-700">
      ...
      <div className={`glass-card shadow-2xl overflow-hidden border-0 ${isDark ? 'bg-white/[0.02]' : 'bg-white/40 border-black/5 shadow-none'}`}>
        <div className={`p-8 border-b flex flex-col md:flex-row md:items-center justify-between gap-6 ${isDark ? 'border-white/5' : 'border-black/5'}`}>
          <div className="flex items-center space-x-6">
            ...
            <div className="flex gap-2">
              <button className={`px-5 py-2 glass-card text-[9px] font-black rounded-xl uppercase tracking-widest ${isDark ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/20' : 'bg-emerald-100 text-emerald-700 border-emerald-500/20'}`}>All Entities</button>
              <button className={`px-5 py-2 glass-card text-[9px] font-black rounded-xl uppercase tracking-widest transition-all ${isDark ? 'bg-white/5 text-white/20 border-white/5 hover:text-white' : 'bg-white text-gray-400 border-black/5 hover:text-black'}`}>Pending Audit</button>
            </div>
          </div>
        ...
        <div className={`p-8 border-t flex justify-between items-center text-[10px] font-bold uppercase tracking-[0.3em] ${isDark ? 'border-white/5 text-white/20' : 'border-black/5 text-gray-400'}`}>
          <p>Displaying {MOCK_DATA.length} Secure Node(s)</p>
          <div className="flex gap-3">
