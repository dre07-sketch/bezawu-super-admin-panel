...
const MOCK_BRANCHES: Branch[] = [
  {
    id: 'b-1',
    supermarket_id: 's-1',
    supermarket_name: 'Freshway Mart',
    name: 'Bole Terminal 01',
    address: 'Bole Road, behind Friendship Mall, Addis Ababa',
    map_pin: 'https://maps.google.com/?q=Bole+Friendship+Addis',
    ...
    manager: {
      id: 'm-1',
      name: 'Dawit Solomon',
      email: 'dawit.s@freshway.et',
      phone: '+251 911 000 111',
      role: 'BRANCH_MANAGER',
      status: 'ACTIVE'
    },
    products: [
      { id: 'p-1', name: 'Fresh Milk', price: 45, unit: 'Litre', sku: 'MLK-001', image_url: '', category: 'Dairy' },
      { id: 'p-2', name: 'Premium Teff', price: 3200, unit: 'Quintal', sku: 'TEF-001', image_url: '', category: 'Grain' }
    ]
  },
  {
    id: 'b-2',
    supermarket_id: 's-1',
    supermarket_name: 'Freshway Mart',
    name: 'Kazanchis Hub 04',
    address: 'Kazanchis, near UNECA, Addis Ababa',
    map_pin: 'https://maps.google.com/?q=Kazanchis+UNECA+Addis',
  ...
  },
  {
    id: 'b-3',
    supermarket_id: 's-2',
    supermarket_name: 'Shoa Shopping',
    name: 'CMC Distribution X',
    address: 'CMC Road, Addis Ababa',
    map_pin: 'https://maps.google.com/?q=CMC+Addis',
    phone: '+251 911 778 899',
    is_busy: false,
    status: 'CLOSED',
...
const Branches: React.FC<BranchesProps> = ({ theme = 'dark' }) => {
  ...
  const toggleBusyMode = (id: string) => {
    setBranches(prev => prev.map(b => b.id === id ? { ...b, is_busy: !b.is_busy } : b));
    if (selectedBranch?.id === id) {
      setSelectedBranch(prev => prev ? { ...prev, is_busy: !prev.is_busy } : null);
    }
  };
  ...
  const toggleStatus = (id: string) => {
    ...
    if (selectedBranch?.id === id) {
      setSelectedBranch(prev => {
        if (!prev) return null;
        const newStatus = prev.status === 'ACTIVE' ? 'CLOSED' : 'ACTIVE';
        ...
        <div className={`p-8 border-b flex flex-col md:flex-row justify-between items-center gap-6 ${isDark ? 'border-white/5 bg-white/[0.02]' : 'border-black/5 bg-gray-50/10'}`}>
          ...
          <div className="flex items-center gap-4">
             <div className={`px-5 py-3 rounded-2xl border flex items-center gap-4 transition-all ${selectedBranch.is_busy ? 'bg-amber-500/10 border-amber-500/30' : 'bg-white/5 border-white/5'}`}>
                ...
                <button 
                   onClick={() => toggleBusyMode(selectedBranch.id)}
               ...
               <section>
                  ...
                  <div className={`p-8 rounded-[2.5rem] border overflow-hidden relative ${isDark ? 'bg-rose-500/[0.02] border-rose-500/10' : 'bg-rose-50 border-rose-100'}`}>
                     ...
                     <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
                        ...
                        <button 
                          onClick={() => toggleStatus(selectedBranch.id)}
                          className={`px-10 py-5 rounded-2xl flex items-center gap-4 transition-all duration-500 font-black uppercase text-[10px] tracking-[0.2em] shadow-2xl ${
                ...
                <tr key={branch.id} className={`transition-all group ${isDark ? 'hover:bg-white/[0.01]' : 'hover:bg-white/30'}`}>
                  ...
                  <td className="px-10 py-7">
                     <div className="flex items-center gap-3">
                        <div className={`w-1.5 h-1.5 rounded-full ${branch.status === 'ACTIVE' ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-rose-500 shadow-[0_0_8px_#f43f5e]'}`}></div>
