...
const MOCK_CUSTOMERS: Customer[] = [
  {
    ...
    car_plate: 'AA 2 12345',
    car_model: 'Toyota Land Cruiser',
    car_color: 'Midnight Black',
    is_verified: true,
    profile_picture: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=256&h=256&fit=crop',
    orders: [
      { 
        id: 'ORD-1010-88', 
        branch_name: 'Bole Terminal 01', 
        status: 'COMPLETED', 
        total_price: 4500.50, 
        car_model: 'Toyota LC', 
        car_plate: 'AA 2 12345', 
        car_color: 'Black', 
        created_at: '2023-11-20 14:30',
        items: [
          { id: 'i-1', name: 'Premium Teff', quantity: 1, price: 3200, unit: 'kg' },
          { id: 'i-2', name: 'Cooking Oil 5L', quantity: 1, price: 1100, unit: 'pcs' },
          { id: 'i-3', name: 'Organic Honey', quantity: 1, price: 200.50, unit: 'pcs' }
        ]
      },
      { 
        id: 'ORD-1052-44', 
        branch_name: 'Kazanchis Hub 04', 
        status: 'COMPLETED', 
        total_price: 1200.00, 
        car_model: 'Toyota LC', 
        car_plate: 'AA 2 12345', 
        car_color: 'Black', 
        created_at: '2023-11-25 09:15',
        items: [
          { id: 'i-4', name: 'Fresh Milk', quantity: 5, price: 200, unit: 'pcs' },
          { id: 'i-5', name: 'Avocado Bag', quantity: 1, price: 200, unit: 'pcs' }
        ]
      }
    ]
  },
  {
    ...
    car_plate: 'OR 3 88721',
    car_model: 'Tesla Model Y',
    car_color: 'Pearl White',
    is_verified: false,
    profile_picture: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=256&h=256&fit=crop',
    orders: [
      { 
        id: 'ORD-2029-12', 
        branch_name: 'Bole Terminal 01', 
        ...
        items: [
          { id: 'i-6', name: 'Blueberries', quantity: 2, price: 400, unit: 'pack' },
          { id: 'i-7', name: 'Almond Milk', quantity: 1, price: 490.75, unit: 'pcs' }
        ]
      }
    ]
  }
...
const Users: React.FC<UsersProps> = ({ theme = 'dark' }) => {
  ...
  const totalSpent = selectedUser?.orders.reduce((acc, curr) => acc + curr.total_price, 0) || 0;
        ...
        <div className={`p-8 border-b flex justify-between items-center ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={`text-[9px] font-black uppercase tracking-widest ${isDark ? 'text-emerald-400/40' : 'text-emerald-600'}`}>{selectedOrder.id}</p>
            </div>
          </div>
        ...
        <div className="p-8 space-y-8">
          <div className="grid grid-cols-2 gap-6">
            <div className={`p-5 rounded-2xl border ${isDark ? 'bg-white/[0.02] border-white/5' : 'bg-gray-50 border-black/5'}`}>
               <p className={`text-[8px] font-black uppercase tracking-widest mb-2 ${isDark ? 'text-white/20' : 'text-gray-400'}`}>Node Origin</p>
            ...
            <div className={`p-5 rounded-2xl border ${isDark ? 'bg-white/[0.02] border-white/5' : 'bg-gray-50 border-black/5'}`}>
               <p className={`text-[8px] font-black uppercase tracking-widest mb-2 ${isDark ? 'text-white/20' : 'text-gray-400'}`}>Timestamp</p>
               <div className="flex items-center gap-2">
                 <Clock size={14} className="text-blue-500" />
                 <span className={`text-xs font-bold ${isDark ? 'text-white/80' : 'text-gray-800'}`}>{selectedOrder.created_at}</span>
               </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-black/20 border-white/5' : 'bg-white border-black/5'}`}>
              <table className="w-full text-left">
                <thead className={`text-[8px] uppercase font-black tracking-widest ${isDark ? 'bg-white/[0.02] text-white/20' : 'bg-gray-50 text-gray-400'}`}>
                  <tr>
                ...
                <tbody className={`text-[10px] font-bold divide-y ${isDark ? 'divide-white/5' : 'divide-black/5'}`}>
                  {selectedOrder.items.map(item => (
                    <tr key={item.id}>
                      <td className={`px-5 py-3 ${isDark ? 'text-white/60' : 'text-gray-700'}`}>{item.name}</td>
                      <td className={`px-5 py-3 text-center ${isDark ? 'text-white/40' : 'text-gray-400'}`}>{item.quantity} {item.unit}</td>
                      <td className={`px-5 py-3 text-right ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>ETB {item.price.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className={`p-6 rounded-3xl flex justify-between items-center ${isDark ? 'bg-emerald-500/5 border border-emerald-500/10' : 'bg-emerald-50 border border-emerald-200'}`}>
            <span className={`text-xs font-black uppercase tracking-widest ${isDark ? 'text-white/40' : 'text-emerald-700'}`}>Total Settlement</span>
            <span className={`text-2xl font-black ${isDark ? 'text-white' : 'text-emerald-900'}`}>ETB {selectedOrder.total_price.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className={`p-10 border-b relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-8 ${isDark ? 'border-white/5 bg-white/[0.01]' : 'border-black/5 bg-gray-50/5'}`}>
          <div className="flex items-center gap-8 z-10">
            ...
            <div>
              ...
              <p className={`text-[10px] font-bold uppercase tracking-[0.5em] ${isDark ? 'text-white/20' : 'text-gray-400'}`}>Customer ID: {selectedUser.id}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-5 space-y-10">
              <section>
                <h4 className={`text-[10px] font-black uppercase tracking-[0.5em] mb-6 ${isDark ? 'text-white/10' : 'text-gray-400'}`}>Identity Protocol</h4>
                <div className={`space-y-4 p-8 rounded-[2rem] border ${isDark ? 'bg-white/[0.01] border-white/5' : 'bg-white/20 border-black/5'}`}>
                   <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? 'bg-white/5 text-emerald-400/30' : 'bg-emerald-50 text-emerald-600'}`}>
                        <Mail size={18} />
                      </div>
                      <span className={`text-sm font-bold ${isDark ? 'text-white/60' : 'text-gray-700'}`}>{selectedUser.email}</span>
                   </div>
                   <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? 'bg-white/5 text-emerald-400/30' : 'bg-emerald-50 text-emerald-600'}`}>
                        <Phone size={18} />
                      </div>
                      <span className={`text-sm font-bold ${isDark ? 'text-white/60' : 'text-gray-700'}`}>{selectedUser.phone}</span>
                   </div>
                </div>
              </section>

              <section>
                <h4 className={`text-[10px] font-black uppercase tracking-[0.5em] mb-6 ${isDark ? 'text-white/10' : 'text-gray-400'}`}>Registered Vehicle</h4>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div className={`p-5 rounded-2xl border ${isDark ? 'bg-black/20 border-white/5' : 'bg-gray-50 border-black/5'}`}>
                         <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[8px] font-black uppercase tracking-widest text-white/20">Plate ID</span>
                         </div>
                      ...
                      <div className={`p-5 rounded-2xl border ${isDark ? 'bg-black/20 border-white/5' : 'bg-gray-50 border-black/5'}`}>
                         <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[8px] font-black uppercase tracking-widest text-white/20">Hue Index</span>
                         </div>
              ...
              <div className={`p-8 rounded-[2rem] border ${isDark ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-emerald-50 border-emerald-100'}`}>
                 ...
                 <p className={`text-[9px] font-medium mt-2 uppercase tracking-tight ${isDark ? 'text-white/20' : 'text-gray-400'}`}>Aggregated platform intake from {selectedUser.orders.length} events.</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-7 space-y-8">
               <div className="flex justify-between items-end">
                  <h4 className={`text-[10px] font-black uppercase tracking-[0.5em] ${isDark ? 'text-white/10' : 'text-gray-400'}`}>Transaction Stream</h4>
                  <p className={`text-[9px] font-black uppercase tracking-widest ${isDark ? 'text-white/20' : 'text-gray-400'}`}>{selectedUser.orders.length} Fiscal Logs</p>
               </div>
               
               <div className={`rounded-[2.5rem] border overflow-hidden ${isDark ? 'bg-white/[0.005] border-white/5' : 'bg-white/20 border-black/5'}`}>
                  <table className="w-full text-left">
                    <thead className={`text-[8px] uppercase font-black tracking-widest ${isDark ? 'bg-white/[0.01] text-white/10' : 'bg-gray-100/30 text-gray-400'}`}>
                       <tr>
                    ...
                    <tbody className={`divide-y ${isDark ? 'divide-white/5' : 'divide-black/5'}`}>
                       {selectedUser.orders.map((o) => (
                         <tr key={o.id} className={`text-[11px] font-bold group/row transition-all ${isDark ? 'text-white/60 hover:bg-white/[0.02]' : 'text-gray-800 hover:bg-gray-50'}`}>
                            <td className={`px-8 py-5 font-mono text-[9px] tracking-widest ${isDark ? 'text-emerald-400/40' : 'text-emerald-600'}`}>{o.id}</td>
                            <td className="px-8 py-5 uppercase tracking-tighter">{o.branch_name}</td>
                            <td className={`px-8 py-5 text-[9px] uppercase ${isDark ? 'text-white/20' : 'text-gray-400'}`}>{o.created_at}</td>
                            <td className="px-8 py-5 font-black">ETB {o.total_price.toLocaleString()}</td>
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-700">
      ...
      <div className={`glass-card overflow-hidden ${isDark ? 'bg-white/[0.005]' : 'bg-white/20 border-black/5 shadow-none'}`}>
                ...
                <tr key={user.id} className={`transition-all group ${isDark ? 'hover:bg-white/[0.01]' : 'hover:bg-white/30'}`}>
                   <td className="px-10 py-7">
                      <div className="flex items-center space-x-5">
                         ...
                         <div>
                            <p className={`text-sm font-black uppercase tracking-tight ${isDark ? 'text-white/80' : 'text-gray-950'}`}>{user.name}</p>
                            <p className={`text-[8px] font-black uppercase tracking-widest ${isDark ? 'text-white/10' : 'text-gray-400'}`}>{user.phone}</p>
                         </div>
                      </div>
                   </td>
                   <td className="px-10 py-7">
                   ...
                   <td className={`px-10 py-7 uppercase font-black tracking-widest ${isDark ? 'text-white/20' : 'text-gray-400'}`}>{user.car_model}</td>
                   ...
                   <td className="px-10 py-7 text-right">
                      <button 
                        ...
                        className={`p-4 rounded-2xl glass-card transition-all ${isDark ? 'bg-white/[0.01] text-white/10 hover:text-emerald-400 hover:bg-white/5' : 'bg-white/60 text-gray-400 hover:text-emerald-600 shadow-sm'}`}
                      >
                        <Eye size={20} />
                      </button>
                   </td>
        ...
        <div className={`p-8 border-t flex justify-between items-center text-[10px] font-bold uppercase tracking-[0.3em] ${isDark ? 'border-white/5 text-white/10' : 'border-black/5 text-gray-400'}`}>
          <p>{MOCK_CUSTOMERS.length} Identity Nodes Syncing</p>
        </div>
      </div>
