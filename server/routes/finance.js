...
// GET /api/finance/payouts - Get pending payouts per supermarket
    ...
    try {
        const payoutsResult = await query(`
            SELECT 
                ...
                ba.account_number
            FROM supermarkets s
            JOIN branches b ON s.id = b.supermarket_id
            JOIN orders o ON b.id = o.branch_id
            LEFT JOIN bank_accounts ba ON s.id = ba.supermarket_id AND ba.is_primary = true
            WHERE o.status != 'CANCELLED' AND o.status != 'cancelled'
            GROUP BY s.id, s.name, ba.bank_name, ba.account_number
